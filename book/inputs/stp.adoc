...
|DA|SA|Len|DSAP=0x42|SSAP=0x42|Ctrl=0x03| BPDU|
...
NOTE: What happens if there is no acknowledgment? xxx
...
|===
| State      | Receive BPDU | Transmit BPDU | Forward data frames | Learn source MAC | Stable?

| Disabled   | No           | No            | No                  | No               | Yes
| Blocking   | Yes          | No            | No                  | No               | Yes
| Listening  | Yes          | Yes           | No                  | No               | No
| Learning   | Yes          | Yes           | No                  | Yes              | No
...
.Task: Set a port as a switchport, set spanning-tree portfast and
...
- Used only if the switch runs legacy STP because it is built in to RSTP802.1w
...
.Task: Set a port as as a switchport, set spanning-tree portfast, and disable port-channeling
----
...
- Cannot be enabled with root guard on the same port
- When configured at the switch level, only monitors Non-Designated ports
- Takes actions on a per-VLAN level
...
+-------------------------------------------+         |
| cGRE Protocol Version Identifier =0x03    |         |
+-------------------------------------------+         |
| cGRE          BPDU Type = 0x02            |         |
+-------------------------------------------+         |
| cGRE             Flags             |                |
